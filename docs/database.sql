...
-- 说明：
-- 1. 本项目只维护业务登录用户和new-api绑定关系。
-- 2. 用户、new-api用户和new-api token由数据库手工维护，本项目不创建new-api用户。
-- 3. 额度、模型、订阅、调用记录、计费、充值码、充值记录、审计日志等能力暂不处理。
...
CREATE TABLE IF NOT EXISTS `user_new_api_binding` (
  ...
  `new_api_user_id` BIGINT NOT NULL COMMENT 'new-api用户ID',
  `new_api_user_name` VARCHAR(128) NOT NULL COMMENT 'new-api用户名称',
  `new_api_api_key` TEXT NOT NULL COMMENT '手工维护的完整明文new-api API Key，格式为sk-xxx',
  `status` VARCHAR(32) NOT NULL DEFAULT 'ENABLE' COMMENT '绑定状态：ENABLE、DISABLE、ERROR',
  `create_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `update_time` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `user_new_api_binding_log` (
  ...
  `operate_type` VARCHAR(64) NOT NULL COMMENT '操作类型：CREATE_BINDING、REFRESH_BINDING、DISABLE_BINDING、RESET_TOKEN、SYNC_FAILED',
