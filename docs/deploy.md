...
- `docker-compose.yml`：启动 `model-gateway-auth`、APISIX、etcd、Dashboard，并自动配置 APISIX 路由。
...
```bash
sed -i "s#^GATEWAY_JWT_PRIVATE_KEY=.*#GATEWAY_JWT_PRIVATE_KEY=$(awk 'NF {sub(/\r/, ""); printf "%s\\n",$0;}' jwt-private.pem)#" .env
sed -i "s#^GATEWAY_JWT_PUBLIC_KEY=.*#GATEWAY_JWT_PUBLIC_KEY=$(awk 'NF {sub(/\r/, ""); printf "%s\\n",$0;}' jwt-public.pem)#" .env
sed -i "s#^GATEWAY_CREDENTIAL_AES_KEY=.*#GATEWAY_CREDENTIAL_AES_KEY=$(cat aes-key.txt)#" .env
sed -i "s#^APISIX_GATEWAY_SECRET=.*#APISIX_GATEWAY_SECRET=$(cat gateway-secret.txt)#" .env
```
